...
export async function blogPostToots(instance, userId, tags = ["blog", "post"]) {
  ...
  let cached = Array.from(cache.entries()).find(
    ([key]) =>
      key.instance === instance &&
      key.userId === userId &&
      key.tags.every((tag) => tags.includes(tag))
  );
  ...
  if (!Array.isArray(data) || data.length === 0) {
    postCache.update((cache) => {
      cache.set(
        { instance, userId, tags },
        { toots: [], lastUpdate: Date.now() }
      );
      return cache;
    });
    return [];
  ...
  postCache.update((cache) => {
    cache.set(
      { instance, userId, tags },
      { toots: data, lastUpdate: Date.now() }
    );
    return cache;
  });
 ...
 * @returns {Promise<Status>}
 */
export async function postRelatedToot(
  instance,
  userId,
  slug,
  tags = ["blog", "post"]
